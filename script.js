...
// 1 - Get Total 
// 2 - Craete new Prodict
// 3 - Save Data in Localstorage
// 4 - Clear Inputs 
// 5 - Add the Prodect in output Read 
// 6 - Count Product
// 7 - Delete Product 
// 8 - Update Product
// 9 - Saerch Product
...
function  getTotel(){
    if (Price.value != "") {
        ...
        Totel.style.backgroundColor = "#040"
    } else {
        Totel.innerHTML = ""
        Totel.style.backgroundColor = "red"
    } 
    
}  

// 2 - Craete new Prodict and Save Data in Localstorage

let DataPro;
if (localStorage.DeeDn != null) {
    DataPro = JSON.parse(localStorage.DeeDn) 
} else {DataPro = []}
...
submit.onclick = function () {
    ...
    if (titel.value != "" && Category.value.toLowerCase() != "" && count.value <= 100) {
        if (mood === "Create") {
        if (newPro.count > 1) {
    ...
    localStorage.setItem("DeeDn" ,JSON.stringify(DataPro))
    showData()
}
...
// 4 - Add the Prodect in output Read 
...
getTotel();
    ...
    for (let i = 0; i < DataPro.length; i++) {
        ...
        <tr>
            ...
            <td><button id="Update" class="Del" onclick = "UpdateData(${i})">Update</button> </td>
            <td><button id="Delete" class="Del" onclick = "deleteData(${i})">Delete</button> </td>
        </tr>`
    }
...
function deleteData(i) {
    ...
    DataPro.splice(i,1)
    localStorage.DeeDn = JSON.stringify(DataPro)
    showData()
}
...
function UpdateData(i) {
    ...
    Totel.innerHTML  =  DataPro[i].Totel
...
//  التي يمكن تنفيذها على البيانات في نظام قاعدة البيانات أو أي نظام يتعامل مع البيانات. يتمثل الاختصار 
//  CRUD هو اختصار للعمليات الأربعة الأساسية في العمليات التالية:

// Create (إنشاء): يتم استخدامها لإنشاء سجل جديد في قاعدة البيانات أو نظام البيانات.
//  عند استخدام العملية 
//   يتم إدخال البيانات الجديدة وتخزينها في الموقع المناسب في النظام.

// Read (قراءة): يتم استخدامها لقراءة أو استرداد البيانات الموجودة في قاعدة البيانات أو النظام.
//  في هذه العملية ،
//  يتم استعراض البيانات المحفوظة واستردادها للعرض أو الاستخدام الآخر.

// Update (تحديث): يستخدم لتحديث أو تعديل البيانات
//  الموجودة في قاعدة البيانات أو النظام. عند استخدام العملية  
//  يتم تحديث البيانات الموجودة بناءً على معلومات جديدة تم تقديمها.

// Delete (حذف): يستخدم لحذف البيانات الموجودة في قاعدة البيانات أو النظام. عند استخدام العملية 
//  يتم إزالة البيانات المحددة من الموقع المناسب في النظام.

// تلك العمليات الأربعة تشكل الأساس لإدارة البيانات في العديد من الأنظمة المختلفة، 
// بما في ذلك قواعد البيانات وتطبيقات الويب وأنظمة إدارة المحتوى وغيرها.
//  توفر هذه العمليات الأربعة طريقة شاملة للتعامل مع البيانات وتحقيق التغييرات والتحكم فيها.
...
// onkeyup                    <===========>  يقع الحدث عندما يقوم المستخدم بتحرير مفتاح على لوحة المفاتيح.
